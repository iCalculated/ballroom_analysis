...
# %% auto 0
__all__ = ['event_names', 'headers', 'name', 'soup', 'events', 'scoresheet_cache', 'Style', 'Event', 'parse_event_name', 'get',
           'get_event_list_html', 'extract_events_from_html', 'Result', 'extract_max_callbacks', 'extract_placement',
           'extract_num_dances', 'get_scoresheet', 'get_event_result']
...
@lru_cache(maxsize=None)
def get_scoresheet(data):
    base_url = "https://results.o2cm.com/scoresheet3.asp"
    response = get(base_url, data=data)
    return response

def get_event_result(name: str, url: str, scoresheet_cache=scoresheet_cache) -> Result:
    ...
    for selector in range(1, possible_callbacks + 1):
        ...
        if dumps(data) in scoresheet_cache:
            soup = scoresheet_cache[dumps(data)]
        else:
            response = session.post(base_url, data=data)
            # Process the response
            soup = BeautifulSoup(response.content, 'html.parser')
            scoresheet_cache[dumps(data)] = soup
